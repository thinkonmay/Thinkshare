...
static void
shared_memory_hub_class_init(SharedMemoryHubMasterClass* klass)
{
...
{
	SharedMemoryHubMaster* self = (SharedMemoryHub*)object;
	SharedMemoryHubMasterPrivate* priv = shared_memory_hub_get_instance_private(self);
...
{
    SharedMemoryHubMasterPrivate* priv = shared_memory_hub_get_instance_private(master);
    ...
    gint id2)
{
    ...
    hub_class->send_data(hub,id1, block->handle);
    hub_class->send_data(hub,id1, block->handle);
