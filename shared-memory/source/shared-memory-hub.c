...
shared_memory_hub_dispose(GObject* object)
{
	...
	{
		g_object_unref(priv->link_array[i]);
	}
}
...
gint data_size)
{
	...
	while (TRUE)
	{
		...
		{
			gint temp;
			g_object_get_property((GObject*)priv->link_array[i],"peer-id",&temp);
			if(temp = peer_id)
			{
				SharedMemoryLinkClass* klass = SHARED_MEMORY_LINK_GET_CLASS(priv->link_array[i]);
				klass->push_message(priv->link_array[i],MESSAGE,data,data_size);
				return TRUE;
			}
		}
		...
		{
			if (priv->hub_id_array[i] == peer_id)
			{
				SharedMemoryLinkClass* klass = SHARED_MEMORY_LINK_GET_CLASS(priv->link_array[i]);
				klass->push_message(priv->link_array[i],MESSAGE,data,data_size);
				check = TRUE;
	...
	gint peer_id)
{
	...
	if(peer_id == 0)
	{
		g_printerr("trying to terminate master link");
		return;
	}
	for (int i = 0; i< MAX_LINK_HUB;i++)
	{
...
gint* 
shared_memory_hub_get_id_array(SharedMemoryHub* self)
{
	SharedMemoryHubPrivate*  priv = shared_memory_hub_get_instance_private(self);

	return priv->hub_id_array;
...
shared_memory_hub_get_link_by_id(SharedMemoryHub* self, gint peer_id)
{
	...
	{
		gint id;
		g_object_get_property((GObject*)priv->link_array[i],"peer-id",&id);
		if (id = peer_id)
		{
...
static void
shared_memory_hub_init(SharedMemoryHub* object)
{
	return;
}
