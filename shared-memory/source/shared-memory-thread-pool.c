...
SharedMemoryMessage* message)
{
   SharedMemoryHub* hub = shared_memory_link_get_owned_hub(link);
   ...
   gint peer_id;
   g_object_get_property((GObject*)link, "peer-id",&peer_id);
...
shared_memory_thread_pool_sender_thread_proxy(gpointer input_data)
{
    ...
    while(TRUE)
    {
        ...
        shared_memory_thread_pool_wait_for_new_task(pool);
...
GError** error)
{
    ...
    if(!success == FALSE)
    {
        ...
        gint id;
        g_object_get_property((GObject*)link,"hub-id",&id);
